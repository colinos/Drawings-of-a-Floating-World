...
int tuioIncrementForThisMachine = 300;
...
//////////////////////////////////
// Dandilines global variables start
//////////////////////////////////

ParticleSystem dandilines_physics;

int dandilines_numParticles = 50;
Particle[] dandilines_mouse = new Particle[maxNumIdsToRecognize];
float dandilines_attractionStrength = -10000;   // magnitude of attraction/repulsion force between mouse and particles
float dandilines_attractionMinDistance = 25;  // "minimum distance that limits how strong this force can get close up"

Particle[] dandilines_others;    // top of dandiline stalk
Particle[] dandilines_anchors;  // invisible anchor particles to mark rest position of 'other' particles
Spring[] dandilines_springs;    // spring force that returns 'other' particles to rest position when mouse is not moving
Particle[] heads;    // dandiline clocks
//Particle[] roots;    // bottom of grass
int[] rootsX;
int[] rootsY;
Particle[] grassTops;    // top of grass
Particle[] grassAnchors;    // invisible anchor particles to mark rest position of 'grassTops' particles 

float dandilines_springStrength = 0.04;  // strength of spring force (how quickly particles will return to rest position)
float dandilines_damping = 0.005;          // strength of oscillation effect when particles are returning to rest position

boolean dandilines_springsActive = true;    // springs become inactive when mouse is moving
float dandilines_periodOfNoMovement = 1;  // length of time before springs come into effect (when mouse is not moving)

float variableXCurve = random(-200, 0); // gives a random variable for dandiline bezier curve
float variableYCurve = random(0, 200); // same as above
float grassXCurve1 = random(-50, 0); // gives a random variable for first grassTop bezier curve
float grassXCurve2 = random(-50, -150); // gives a random variable for second grassTop bezier curve
float grassYCurve = random(100, 200); // same as above
float rootXCurve = random(-150, -50); // number generated for y bezier on root must be negative
float rootYCurve = random(-150, -50); // number generated for y bezier on root must be negative

PImage dandilines_img;

//////////////////////////////////
// Dandilines global variables end
//////////////////////////////////
...
//////////////////////////////////
// RotatingLeaves global variables start
//////////////////////////////////

ParticleSystem leaves_physics;

int leaves_numParticles = 500;
Particle[] leaves_mouse = new Particle[maxNumIdsToRecognize];
float leaves_attractionStrength = -7500;   // magnitude of attraction/repulsion force between mouse and particles
float leaves_attractionMinDistance = 80;  // "minimum distance that limits how strong this force can get close up"

Particle[] leaves_others;
Particle[] leaves_anchors;  // invisible anchor particles to mark rest position of 'other' particles
Spring[] leaves_springs;    // spring force that returns 'other' particles to rest position when mouse is not moving
float leaves_springStrength = 0.001;  // strength of spring force (how quickly particles will return to rest position)
float leaves_damping = 0.004;          // strength of oscillation effect when particles are returning to rest position

float[] leaves_angles;
float[] leaves_angleIncrements;
float[] leaves_rotationAsFractionOfMaxSpeed;  // represents current rotation speed as percentage of maximum rotation speed
// used when slowing down rotating leaves
//float[] scales;

boolean leaves_springsActive = true;    // springs become inactive when mouse is moving
float leaves_periodOfNoMovement = 1;  // length of time before springs come into effect (when mouse is not moving)

PImage leaves_img;

//////////////////////////////////
// RotatingLeaves global variables end
//////////////////////////////////

//////////////////////////////////
// SpringDots global variables start
//////////////////////////////////

ParticleSystem springDots_physics;

int springDots_numParticles = 20;
Particle[] springDots_mouse = new Particle[maxNumIdsToRecognize];
float springDots_attractionStrength = -10000;   // magnitude of attraction/repulsion force between mouse and particles
float springDots_attractionMinDistance = 100;  // "minimum distance that limits how strong this force can get close up"

Particle[][] springDots_others;
Particle[][] springDots_anchors;  // invisible anchor particles to mark rest position of 'other' particles
Spring[][] springDots_springs;    // spring force that returns 'other' particles to rest position when mouse is not moving

float springDots_springStrength = 0.005;  // strength of spring force (how quickly particles will return to rest position)
float springDots_damping = 0.003;          // strength of oscillation effect when particles are returning to rest position

boolean springDots_springsActive = true;    // springs become inactive when mouse is moving
float springDots_periodOfNoMovement = 1;  // length of time before springs come into effect (when mouse is not moving)

PImage springDots_img;

//////////////////////////////////
// SpringDots global variables end
//////////////////////////////////
...
void draw()
{
  ...
  // Below block just added because x-dimension on one screen was reversed
  for (int j = 0; j < maxNumIdsToRecognize; j++) {
    if (touchX[j] > width/2) {
      touchX[j] = width/2 - (touchX[j] - width/2);
    } else {
      touchX[j] = width/2 + (width/2 - touchX[j]);
    }
  }

  switch(idOfCurrentDrawFunction) {
  case 0: 
    draw0(tuioCursorList);  // Dandilines
    if (fadingOut) {
      fadeOut(0);
    }
    break;
  ...
  */
  case 1: 
    draw2(tuioCursorList);  // RotatingLeaves
    if (fadingOut) {
      fadeOut(0);
    }
    break;
  case 2: 
    draw3(tuioCursorList);  // SpringDots
    if (fadingOut) {
      fadeOut(0);
    }
    break;
