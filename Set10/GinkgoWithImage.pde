void setup1() {
  ginkgo_ginkgo= loadImage("ginkgo.png");
  colorMode(RGB,255,255,255,100);
  ginkgo_psystems = new ArrayList();
}
...
void draw1(TuioCursor[] tuioCursorList) {
  ...
  for (int k = 0; k < maxNumIdsToRecognize; k++) {
    ...
    if((touchX[k] != previousX[k])||(touchY[k] != previousY[k])){
      ginkgo_psystems.add(new ParticleSystem(int(random(1,5)),new Vector3D(touchX[k],touchY[k])));
    }
  }  ///this one
   
  // Cycle through all particle systems, run them and delete old ones
  for (int i = ginkgo_psystems.size()-1; i >= 0; i--) {
    ParticleSystem psys = (ParticleSystem) ginkgo_psystems.get(i);
    psys.run();
    if (psys.dead()) {
      ginkgo_psystems.remove(i);
    }
  }
}
...
class CrazyParticle extends Particle {
  ...
  void render() {
    ...
    rotate(theta);
    //stroke(255,timer);
     tint(orange,timer); //fade
    //line(0,0,25,0);
    imageMode(CORNER);
    image(ginkgo_ginkgo,0,0,75,90);
